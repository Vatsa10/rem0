...
class GreetingCache:
    ...
    Stores mulaw 8kHz audio bytes keyed by (language, company, agent).
    Lookups are O(1) — audio is ready to stream directly to Twilio.
    """
    ...
    def _key(self, language: str, company: str, agent: str) -> str:
        h = hashlib.md5(f"{company}|{agent}".encode()).hexdigest()[:8]
        return f"greeting:{language}:{h}"

    async def get(
        self, language: str, company: str, agent: str
    ) -> Optional[bytes]:
        """Get cached greeting audio (mulaw 8kHz). Returns None if not cached."""
        if not self._client:
            return None
        try:
            return await self._client.get(self._key(language, company, agent))
        except Exception as e:
            logger.warning(f"Cache get failed: {e}")
            return None

    async def set(
        self, language: str, company: str, agent: str, audio: bytes,
        ...
        try:
            await self._client.set(
                self._key(language, company, agent), audio, ex=ttl_seconds
            )
            logger.info(
                f"Cached greeting: lang={language}, bytes={len(audio)}"
            )
        except Exception as e:
    ...
    async def get_text(
        ...
        if not self._client:
            return None
        try:
            key = self._key(language, company, agent) + ":text"
    ...
    async def set_text(
        self, language: str, company: str, agent: str, text: str,
        ttl_seconds: int = 86400 * 7,
    ) -> None:
        if not self._client:
            return
